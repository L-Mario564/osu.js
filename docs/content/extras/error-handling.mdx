...
To handle errors thrown while using the library, osu.js provides the `OsuJSGeneralError` and `OsuJSUnexpectedResponseError` classes and a type of `OsuJSError`. The classes are thrown as exceptions while type is simple a union of both of these classes.
