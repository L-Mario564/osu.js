...
export async function getExistingAccessToken() {
  if (!existsSync(tokenFileLocation)) {
    throw new Error('No file containing the API token was found. To generate this file, run "npm run test:auth" and completing the authorization code grant flow');
  }
